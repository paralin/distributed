...
import (
	...
	"github.com/fuserobotics/distributed/pkg/httputils"
	"github.com/fuserobotics/distributed/pkg/ioutils"
	"github.com/fuserobotics/distributed/pkg/stringid"
	"github.com/fuserobotics/distributed/pkg/tarsum"
...
func (r *Session) GetRepositoryData(name reference.Named) (*RepositoryData, error) {
	repositoryTarget := fmt.Sprintf("%srepositories/%s/images", r.indexEndpoint.String(), name.Name())
	...
	if res.StatusCode == 404 {
		...
		return nil, httputils.NewHTTPRequestError(fmt.Sprintf("Error: Status %d trying to pull repository %s: %q", res.StatusCode, name.Name(), errBody), res)
	}
...
func (r *Session) PushImageJSONIndex(remote reference.Named, imgList []*ImgData, validate bool, regs []string) (*RepositoryData, error) {
	...
	u := fmt.Sprintf("%srepositories/%s/%s", r.indexEndpoint.String(), remote.Name(), suffix)
